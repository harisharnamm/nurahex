...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (problemRef.current) {
      ...
      gsap.fromTo(
        problemRef.current.querySelector("img"),
        { opacity: 0, scale: 0.8 },
        { 
          opacity: 1, 
          scale: 1, 
  ...
  useEffect(() => {
    ...
    // Enhanced Pinned Section with 3D Content Reveal
    if (pinnedSectionRef.current) {
      const pinnedItems = pinnedSectionRef.current.querySelectorAll('.pinned-item')
      if (pinnedItems.length > 0) {
        
        // Detect device capabilities for performance optimization
        const isMobile = window.innerWidth < 768
        const supportsAdvanced3D = !isMobile && !window.matchMedia('(prefers-reduced-motion: reduce)').matches
        
        // Adjust animation complexity based on device capabilities
        const zDepth = supportsAdvanced3D ? -400 : -200
        const maxZForward = supportsAdvanced3D ? 300 : 150
        const rotationIntensity = supportsAdvanced3D ? 25 : 15
        const blurAmount = supportsAdvanced3D ? '8px' : '4px'
        
        // Set initial state - all items invisible with enhanced 3D transforms
        gsap.set(pinnedItems, { 
          opacity: 0,
          z: zDepth,
          rotationX: rotationIntensity,
          rotationY: supportsAdvanced3D ? 10 : 5,
          scale: 0.6,
          y: 80,
          filter: `blur(${blurAmount})`,
          transformOrigin: 'center center'
        })
        gsap.set(pinnedItems[0], { 
          opacity: 1,
          z: 0,
          rotationX: 0,
          rotationY: 0,
        ...
        const tl = gsap.timeline({
          scrollTrigger: {
            ...
            scrub: supportsAdvanced3D ? 3 : 2, // Adjust scrub based on device
            pin: true,
            anticipatePin: 1,
        ...
        // Create cinematic 3D transitions between items
        pinnedItems.forEach((item, index) => {
          if (index === 0) {
            // First item: dramatic exit with enhanced 3D effect
            tl.to(item, {
              opacity: 0,
              z: maxZForward,
              rotationX: -rotationIntensity * 0.8,
              rotationY: supportsAdvanced3D ? -8 : -4,
              scale: 1.2,
              y: -80,
              filter: `blur(${supportsAdvanced3D ? '6px' : '3px'})`,
              duration: supportsAdvanced3D ? 1.8 : 1.2,
              ease: 'power3.inOut'
            }, 2.2)
            return
          }
          
          // Subsequent items: enhanced cinematic transitions
          const startTime = index * (supportsAdvanced3D ? 3.5 : 2.8) // Adjust timing
          const holdTime = supportsAdvanced3D ? 2.5 : 2.0 // Adjust hold time
          
          // Transition IN: dramatic entrance from deep background
          tl.to(item, {
            opacity: 1,
            z: 0,
            rotationX: 0,
          ...
          // Hold in place with subtle breathing effect (only on capable devices)
          if (supportsAdvanced3D) {
            tl.to(item, {
              opacity: 1,
              z: 0,
              rotationX: 0,
            ...
            .to(item, {
              ...
              y: 0,
              filter: 'blur(0px)',
              duration: holdTime * 0.4,
              ease: 'none'
            }, startTime + 1.8 + holdTime * 0.6)
          } else {
            // Simpler hold for mobile
            tl.to(item, {
              opacity: 1,
              z: 0,
              rotationX: 0,
          ...
          // Transition OUT: dramatic exit to foreground (unless it's the last item)
          if (index < pinnedItems.length - 1) {
            tl.to(item, {
              ...
              duration: supportsAdvanced3D ? 1.8 : 1.2,
              ease: 'power3.in'
            }, startTime + (supportsAdvanced3D ? 1.8 + holdTime : 1.2 + holdTime))
          } else {
            // Last item: extended graceful exit
            tl.to(item, {
              opacity: 0,
              z: maxZForward * 1.1,
              rotationX: -rotationIntensity,
              rotationY: supportsAdvanced3D ? -10 : -5,
              scale: supportsAdvanced3D ? 1.3 : 1.2,
              y: supportsAdvanced3D ? -100 : -80,
              filter: `blur(${supportsAdvanced3D ? '10px' : '5px'})`,
              duration: supportsAdvanced3D ? 2.2 : 1.6,
              ease: 'power4.in'
            }, startTime + (supportsAdvanced3D ? 1.8 + holdTime + 1.5 : 1.2 + holdTime + 1.0))
          }
        })
        
        // Add dynamic background effects synchronized with text transitions (advanced devices only)
        if (supportsAdvanced3D) {
          const backgroundOverlay = pinnedSectionRef.current.querySelector('.bg-gradient-radial')
          if (backgroundOverlay) {
            tl.to(backgroundOverlay, {
              opacity: 0.6,
              scale: 1.1,
              duration: 1,
              ease: 'sine.inOut',
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div id="main-content" style={{ opacity: loading ? 0 : 1, transition: 'opacity 0.3s' }}>
          ...
          <div className="container mx-auto px-6">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div className="space-y-8">
                <AnimatedText animation="splitWords" className="text-3xl md:text-4xl font-bold">
                  AI implementation <span className="text-primary-700">without the headaches</span>
                </AnimatedText>
                <AnimatedText animation="slideUp" delay={0.3} className="text-lg text-surface-50/80">
        ...
        <section ref={pinnedSectionRef} className="pinned-section relative h-[240vh] bg-surface-950">
          {/* Enhanced dynamic background overlay */}
          <div className="absolute inset-0 bg-gradient-to-b from-surface-950/90 via-surface-900/70 to-surface-950/90 z-0"></div>
          <div className="absolute inset-0 bg-gradient-radial from-primary-700/8 via-transparent to-transparent opacity-40 z-0"></div>
          <div className="absolute inset-0 bg-gradient-radial from-accent-600/3 via-transparent to-transparent opacity-50 z-0"></div>
          
          <div className="sticky top-0 h-screen flex items-center justify-center overflow-hidden relative z-10">
            <div className="container mx-auto px-6 text-center">
              <div className="pinned-content relative">
                <div className="pinned-item absolute inset-0 flex flex-col items-center justify-center">
                  <h2 className="text-4xl md:text-6xl font-bold mb-6 transform-gpu drop-shadow-3d">
                    <span className="text-[#D7FF1E]">Revolutionary</span> AI Solutions
                  </h2>
                  <p className="text-xl text-surface-50/90 max-w-3xl mx-auto leading-relaxed transform-gpu">
                    We don't just implement AI - we revolutionize how your business operates
                  </p>
                </div>
                <div className="pinned-item absolute inset-0 flex flex-col items-center justify-center">
                  <h2 className="text-4xl md:text-6xl font-bold mb-6 transform-gpu drop-shadow-3d">
                    <span className="text-[#D7FF1E]">Lightning</span> Fast Deployment
                  </h2>
                  <p className="text-xl text-surface-50/90 max-w-3xl mx-auto leading-relaxed transform-gpu">
                ...
                <div className="pinned-item absolute inset-0 flex flex-col items-center justify-center">
                  <h2 className="text-4xl md:text-6xl font-bold mb-6 transform-gpu drop-shadow-3d">
                    <span className="text-[#D7FF1E]">Measurable</span> Results
                  </h2>
                  <p className="text-xl text-surface-50/90 max-w-3xl mx-auto leading-relaxed transform-gpu">
                    Proven ROI with enterprise-grade solutions that scale
                  </p>
                </div>
              </div>
            </div>
        ...
        <section ref={servicesRef} className="py-16 bg-surface-900 relative overflow-hidden">
          ...
          <div className="container mx-auto px-6 relative z-10">
            <div className="text-center mb-16">
              <AnimatedText animation="splitWords" className="text-3xl md:text-4xl font-bold mb-4">
                Our <span className="text-primary-700">Services</span>
              </AnimatedText>
              <AnimatedText animation="fadeIn" delay={0.2} className="text-lg text-surface-50/80 max-w-2xl mx-auto">
        ...
        <section ref={socialProofRef} className="py-16 bg-surface-700 overflow-hidden">
          <div className="container mx-auto px-6">
            <div className="text-center mb-12">
              <AnimatedText animation="splitWords" className="text-3xl font-bold mb-4">
            ...
            <div className="relative">
              <div className="flex space-x-12 animate-marquee">
                {Array.from({ length: 8 }).map((_, index) => (
                  <div key={index} className="flex items-center justify-center w-40 h-20 bg-surface-600/50 rounded-lg">
                    <div className="w-24 h-8 bg-surface-400/50 rounded"></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mt-16 text-center">
              <AnimatedText animation="fadeIn" trigger="top 90%" className="text-xl italic text-surface-50/80 max-w-3xl mx-auto">
                "NurahexAI transformed our customer support operations with their AI solution. We've seen a 70% reduction in
                response time and significantly improved customer satisfaction."
              </AnimatedText>
              <div className="mt-4">
                <AnimatedText animation="slideUp" delay={0.3} className="font-medium">Sarah Johnson</AnimatedText>
                <AnimatedText animation="slideUp" delay={0.4} className="text-sm text-surface-50/60">CTO, TechCorp</AnimatedText>
              </div>
            </div>
          </div>
        </section>
