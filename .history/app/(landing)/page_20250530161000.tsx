  ...
  useEffect(() => {
    ...
    if (pinnedSectionRef.current) {
      ...
      if (pinnedItems.length > 0) {
        ...
        const zDepth = supportsAdvanced3D ? -300 : -150
        const maxZForward = supportsAdvanced3D ? 200 : 100
        const rotationIntensity = supportsAdvanced3D ? 20 : 10
        
        // Set initial state - first item visible, others hidden but not too far
        pinnedItems.forEach((item, index) => {
          if (index === 0) {
            ...
            gsap.set(item, { 
              ...
              scale: 0.8,
              y: 50,
              filter: supportsAdvanced3D ? 'blur(4px)' : 'blur(2px)',
              transformOrigin: 'center center'
            })
          }
        })
        ...
        const tl = gsap.timeline({
          scrollTrigger: {
            ...
            scrub: 2, // Consistent scrub value
            pin: true,
            anticipatePin: 1,
            invalidateOnRefresh: true,
            onUpdate: function(self) {
              // Debug logging to see if ScrollTrigger is working
        ...
        pinnedItems.forEach((item, index) => {
          if (index === 0) {
            // First item: exit animation
            tl.to(item, {
              opacity: 0,
              z: maxZForward,
              rotationX: -rotationIntensity * 0.7,
              rotationY: supportsAdvanced3D ? -6 : -3,
              scale: 1.15,
              y: -60,
              filter: supportsAdvanced3D ? 'blur(3px)' : 'blur(1px)',
              duration: 1.5,
              ease: 'power2.inOut'
            }, 1.5)
            return
          ...
          // Calculate timing for subsequent items
          const startTime = index * 2.8
          const holdDuration = 2.0
          
          // Entry animation: bring item into view
          tl.to(item, {
            opacity: 1,
            z: 0,
            rotationX: 0,
          ...
          if (index < pinnedItems.length - 1) {
            tl.to(item, {
              ...
              duration: 1.2,
              ease: 'power2.in'
            }, startTime + 1.2 + holdDuration)
          } else {
            // Last item stays longer then fades out
            tl.to(item, {
              opacity: 0,
              z: maxZForward,
              rotationX: -rotationIntensity,
              ...
              scale: 1.2,
              y: -80,
              filter: supportsAdvanced3D ? 'blur(5px)' : 'blur(2px)',
              duration: 1.8,
              ease: 'power3.in'
            }, startTime + 1.2 + holdDuration + 1.0)
          }
        })
      }
    }
