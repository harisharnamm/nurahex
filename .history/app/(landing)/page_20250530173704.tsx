...
import ScrollFloat from "@/components/home/scroll-float"
import AIHighlights from "@/components/home/ai-highlights"
import "@/components/home/ai-highlights.css"
...
export default function Home() {
  ...
  // --- Scroll-pinned crossfade section logic ---
  useLayoutEffect(() => {
    // Clear any existing ScrollTrigger instances to prevent conflicts
    ScrollTrigger.getAll().forEach(trigger => trigger.kill());
    
    const slides = aiHighlightsRef.current?.querySelectorAll('.ai-highlight-slide') ?? [];
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        scrub: 0.5,
        pin: true, // Pin the whole section instead of just slides
        pinSpacing: true,
        anticipatePin: 1,
        markers: false, // Set to true for debugging
        toggleActions: "restart none none none",
        invalidateOnRefresh: true,
        onEnter: () => console.log("Entered ai-highlights section"),
        onLeave: () => console.log("Left ai-highlights section")
      }
    });
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div id="main-content" style={{ opacity: loading ? 0 : 1, transition: 'opacity 0.3s' }}>
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div className="space-y-8">
                ...
                <ul className="space-y-4">
                  {["Eliminate engineering bottlenecks","Reduce time-to-market by 70%","Production-ready, not proof-of-concept","Seamless integration with existing systems","Ongoing optimization and support",].map((item, index) => (
        ...
        {/* Scroll-pinned crossfade section: only one slide visible at a time */}
        <section
          id="ai-highlights"
          className="relative h-[300vh] bg-surface-950 flex items-center justify-center"
        >
          <div className="sticky top-0 h-screen flex items-center justify-center w-full">
            <div ref={aiHighlightsRef} className="w-full max-w-2xl mx-auto relative" id="ai-highlights-slides">
              {[
                {
                ...
                <div
                  key={slide.headline}
                  className="ai-highlight-slide absolute inset-0 flex flex-col items-center justify-center text-center opacity-0 pointer-events-none"
                >
                  <h2 className="text-4xl md:text-6xl font-bold mb-4 text-[#D7FF1E]">{slide.headline}</h2>
                  <p className="text-lg md:text-xl text-surface-50/90 max-w-xl mx-auto">{slide.subtext}</p>
                </div>
              ))}
            </div>
          </div>
