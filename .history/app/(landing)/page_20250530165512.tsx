...
import PinnedHighlights from "@/components/home/pinned-highlights"
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Enhanced Pinned Section with Smooth Content Reveal
    if (pinnedSectionRef.current) {
      ...
      if (pinnedItems.length > 0) {
        ...
        // Use more conservative transform values to prevent rendering issues
        ...
        // Set initial state with safer transforms
        pinnedItems.forEach((item, index) => {
          if (index === 0) {
            ...
            gsap.set(item, { 
              ...
              rotationY: supportsAdvanced3D ? 3 : 2,
              scale: 0.85,
              y: 30,
              filter: supportsAdvanced3D ? 'blur(2px)' : 'blur(1px)',
              transformOrigin: 'center center'
            })
          }
        })
        ...
        // Smoother overlap and extra scroll space after last text
        const transitionDuration = 2.5
        const itemDuration = 3.5
        const overlapAmount = 1.2 // increased for more crossfade
        const extraScrollHold = 2.5 // seconds to hold after last text
        
        pinnedItems.forEach((item, index) => {
          if (index === 0) {
            // First item: hold then fade out with overlap
            tl.to(item, {
              opacity: 1,
            ...
            .to(item, {
              opacity: 0,
              z: maxZForward * 0.5,
              rotationX: -rotationIntensity * 0.3,
              scale: 1.1,
              y: -30,
              filter: supportsAdvanced3D ? 'blur(1px)' : 'blur(0.5px)',
              duration: transitionDuration,
              ease: 'power2.inOut'
            }, itemDuration - overlapAmount)
            return
          }
          
          // Calculate overlapping timing to ensure smooth transitions
          const startTime = (index * itemDuration) - (overlapAmount * index)
          
          // Entry animation: smooth transition from hidden to visible
          tl.to(item, {
            opacity: 1,
            z: 0,
            rotationX: 0,
          ...
          .to(item, {
            ...
            y: 0,
            filter: 'blur(0px)',
            duration: itemDuration - transitionDuration,
            ease: 'none'
          }, startTime + (transitionDuration * 0.6))
          
          // Exit animation (except for last item)
          if (index < pinnedItems.length - 1) {
            tl.to(item, {
              ...
              duration: transitionDuration,
              ease: 'power2.inOut'
            }, startTime + itemDuration - overlapAmount)
          } else {
            // Last item: hold, then fade out, then hold blank for extra scroll
            tl.to(item, {
              opacity: 1,
              duration: 1.5,
              ease: 'none'
            }, startTime + itemDuration - transitionDuration)
            .to(item, {
              opacity: 0,
              z: maxZForward,
              rotationX: -rotationIntensity * 0.5,
            ...
            // Add a blank hold after last text for extra scroll space
            tl.to({}, { duration: extraScrollHold }, startTime + itemDuration + 2.5)
          }
        })
      }
    }
