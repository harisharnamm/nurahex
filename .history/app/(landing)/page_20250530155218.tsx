  ...
  useEffect(() => {
    ...
    if (pinnedSectionRef.current) {
      ...
      if (pinnedItems.length > 0) {
        // Set initial state - all items invisible except first
        gsap.set(pinnedItems, { opacity: 0 })
        gsap.set(pinnedItems[0], { opacity: 1 })
        ...
        pinnedItems.forEach((item, index) => {
          if (index === 0) return // First item starts visible
          
          // Fade out previous item and fade in current item
          tl.to(pinnedItems[index - 1], {
            opacity: 0,
            duration: 0.5,
            ease: 'power2.inOut'
          }, index * 1.5)
        ...
        tl.to(pinnedItems[pinnedItems.length - 1], {
          opacity: 0,
          duration: 0.5,
          ease: 'power2.inOut'
        }, (pinnedItems.length - 1) * 1.5 + 1)
      }
    }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div id="main-content" style={{ opacity: loading ? 0 : 1, transition: 'opacity 0.3s' }}>
        ...
        {/* Pinned Section with Content Reveal */}
        <section ref={pinnedSectionRef} className="pinned-section relative h-[300vh] bg-surface-950">
