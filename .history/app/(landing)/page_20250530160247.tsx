  ...
  useEffect(() => {
    ...
    if (pinnedSectionRef.current) {
      ...
      if (pinnedItems.length > 0) {
        // Set initial state - all items invisible with 3D transforms
        ...
        const tl = gsap.timeline({
          scrollTrigger: {
            ...
            scrub: 2, // Slower scrub for smoother control
            pin: true,
            anticipatePin: 1
          }
        })
        
        // Create smooth 3D transitions between items
        pinnedItems.forEach((item, index) => {
          if (index === 0) {
            // First item: stay visible then transition out
            tl.to(item, {
              opacity: 0,
              z: 200,
              rotationX: -15,
          ...
          // Subsequent items: transition in from back, hold, then transition out
          const startTime = index * 2.5
          const holdTime = 1.8
          
          // Transition IN: from back to front
          tl.to(item, {
            opacity: 1,
            z: 0,
            rotationX: 0,
          ...
          // Transition OUT: from front to forward (unless it's the last item)
          if (index < pinnedItems.length - 1) {
            tl.to(item, {
              ...
              duration: 1.2,
              ease: 'power2.in'
            }, startTime + 1.2 + holdTime)
          } else {
            // Last item: fade out at the very end
            tl.to(item, {
              opacity: 0,
              z: 200,
              rotationX: -15,
