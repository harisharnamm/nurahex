...
import { Check, ArrowRight, Bot, Globe, Brain, Sparkles, LineChart, Layers, Zap, Target, Rocket } from "lucide-react"
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Feature cards section animations
  ...
  useEffect(() => {
    ...
    featureCards.forEach(card => {
      ...
      const supportingText = cardElement.querySelector('.supporting-text')
      const headText = cardElement.querySelector('.head-text')
      ...
      cardElement.addEventListener('mouseenter', () => {
        ...
        gsap.to(cardElement, {
          y: -12,
          scale: 1.02,
          boxShadow: "0 25px 50px rgba(215, 255, 30, 0.15), 0 0 40px rgba(215, 255, 30, 0.1)",
          duration: 0.4,
          ease: "power2.out"
        })
        ...
        // Show supporting text with slide up effect
        ...
        <section className="py-16 bg-surface-950 relative overflow-hidden">
          <div className="container mx-auto px-6">
            <div className="feature-cards-container grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                {
                  icon: <Rocket className="h-10 w-10 text-primary-700" />,
                  head: "Revolutionary AI Solutions",
                  supporting: "We don't just implement AI - we revolutionise how your business operates"
                },
                {
                  icon: <Zap className="h-10 w-10 text-primary-700" />,
                  head: "Lightning Fast Deployment",
                ...
                },
                {
                  ...
                  supporting: "Proven ROI with enterprise-grade solutions that scale"
                }
              ].map((feature, index) => (
                <CardGlass key={index} className="feature-card relative h-48 p-8 cursor-pointer overflow-hidden group">
                  {/* Background gradient overlay for hover effect */}
                  <div className="absolute inset-0 bg-gradient-to-br from-primary-700/5 to-accent-600/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                  
                  <div className="relative z-10 h-full flex flex-col justify-between">
                    {/* Icon */}
                    <div className="feature-icon w-16 h-16 rounded-full bg-primary-700/20 flex items-center justify-center mb-6 backdrop-blur-sm border border-primary-700/30">
                      {feature.icon}
                    </div>
                    
                    {/* Content */}
                    <div className="flex-1">
                      <h3 className="head-text text-xl md:text-2xl font-bold mb-4 leading-tight">
                        {feature.head}
                      </h3>
                      
                      {/* Supporting text - hidden by default, shown on hover */}
                      <p className="supporting-text text-surface-50/80 text-base leading-relaxed opacity-0 transform translate-y-2">
                        {feature.supporting}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="absolute inset-0 rounded-lg border border-transparent group-hover:border-primary-700/20 transition-all duration-300"></div>
                </CardGlass>
              ))}
            </div>
          </div>
